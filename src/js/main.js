...
let Filter = function () {
    let input = document.getElementById('manufacturer');
    let selectValue = document.getElementsByClassName("selected-values");
    input.addEventListener('keyup', function () {
        let filter = input.value.toLowerCase(),
            filterName = document.querySelectorAll('.products-item');
        
            filterName.forEach(item => {
            if (item.innerHTML.toLowerCase().indexOf(filter) > -1) {
                item.style.display = '';
                selectValue[0].innerHTML = filter;
                if(selectValue[0].innerHTML.length > 0){
                    selectValue[0].classList.add("active");
                }else if(selectValue[0].innerHTML.length <= 0){
                    selectValue[0].classList.remove("active");
                }
            } else {
    ...
    const filterBox = document.querySelectorAll('.products-item');

    document.querySelector('.btn-wrapper').addEventListener('click', (event) => {
